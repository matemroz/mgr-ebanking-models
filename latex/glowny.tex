...
Komunikacja pomiędzy warstwami: biznesową, aplikacyjną i technologiczną stanowi jedną z największych zalet ArchiMate (rys. \ref{archimate_concepts}).
...
\begin{figure}[h!tbp]
\begin{centering}
\includegraphics[width=14cm]{img/archimate_concepts.png}
\caption[Koncpecja ArchiMate z wyraźnym podziałem na warswy: biznesową, aplikacji i technologiczną.]{Koncpecja ArchiMate z wyraźnym podziałem na warswy: biznesową, aplikacji i technologiczną. \cite{ArchWhatIs}}\label{archimate_concepts}
\end{centering}
\end{figure}
...
Następnie powinno zidentyfikować się przepływy pomiędzy różnymi encjami \emph{Business Model Canvas} oraz dokonać mapowania ogólnego modelu biznesu jako usług powiązanych. W ArchiMate, na najwyższym poziomie, organizacja i odpowiadający jej model biznesowy może być reprezentowany jak pojedyncza usługa biznesowa (ang. \emph{Business Service}). Segmenty klientów i kluczowi partnerzy mogą być reprezentowani jako encje aktorów biznesowych (ang. \emph{Business actors}). Każdy z nich powinien mieć przypisaną co najmniej jedną rolę biznesową (ang. \emph{Business role}) i być połączonym z usługą organizacji przez biznesowe relacje interfejsu (ang. \emph{Business interface relationships}) (rys. \ref{remap_overall_business}). 
\begin{figure}[h!tbp]
\begin{centering}
\includegraphics[width=14cm]{img/remap_overall_business.png}
\caption[Relacje biznesowe aktorów i usługi w organizacji.]{Relacje biznesowe aktorów i usługi w organizacji. \cite{TopDownArchimate}}\label{remap_overall_business}
\end{centering}
\end{figure}

W kolejnym kroku rozwijane są szczegóły ogólnego modelu biznesowego. Następuje przejście z \emph{Business Model Canvas} na \emph{Enterprise Canvas}. \ref{enterprise_canvas_map}

\begin{figure}[h!tbp]
\begin{centering}
\includegraphics[width=12cm]{img/enterprise_canvas_map.png}
\caption[Mapowanie \emph{Business Model Canvas} na \emph{Enterprise Canvas}.]{Mapowanie \emph{Business Model Canvas} na \emph{Enterprise Canvas}. \cite{TopDownArchimate}}\label{enterprise_canvas_map}
\end{centering}
\end{figure}
