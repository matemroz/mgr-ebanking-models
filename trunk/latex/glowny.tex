...
W celu dokładnego opisania metodyki zostaną przedstawione jej poszczególne fazy oraz związane z nią notacje modelowania. Będą opisane również możliwe narzędzia, które wspierają proces projektowania w oparciu o MatSOA.

\subsection{Fazy metodyki}
Metodyka MatSOA składa się z sześciu głównych faz, których sekwencja została przedstawiona na rys. \ref{soam_lifecycle}. 

Faza analizy ma równoległy przebieg. Jednocześnie w trakcie analizy organizacji oraz wykrywania usług zachodzi analiza obecnych systemów. Obydwa podejścia analizy - \emph{top-down} oraz \emph{bottom-up} - są następnie konsolidowane. W kolejnym etapie projektowane są usługi, które z kolei są wykorzystywane do przygotowania procesów biznesowych.

\begin{figure}[h!tbp]
\begin{centering}
\includegraphics[width=12cm]{img/SOAM.png}
\caption[Fazy metodyki MatSOA.]{Fazy metodyki MatSOA.}\label{soam_lifecycle}
\end{centering}
\end{figure}

W tej sekcji pracy zostaną omówione poszczególne fazy metodyki MatSOA.


\subsection*{Analiza organizacji}
W celu identyfikacji wymagań organizacji są brane pod uwagę takie elementy jak: strategie biznesowe, procesy biznesowe oraz domeny funkcjonalne. Wszystkie aktywności włączone w tę fazę są rozpatrywane przez analityka biznesowego. 

W pierwszym opcjonalnym kroku analizy organizacji w metodzie MatSOA tworzony jest BMM \emph{Business motivation model}. Stanowi on część opisu organzacji. Umożliwia uchwycenie i przedstawienie w prosty sposób planów biznesowych. Do podstawowych celów BMM zalicza się:
\begin{itemize}
\item{identyfikację czynników, które motywują powstanie planu,}
\item{identyfikację oraz definicję elementów oraz planu,}
\item{określenie, jak wymienione elementy są ze sobą powiązane.}
\end{itemize}

Grupa OMG jako pierwsza poddała BMM formalnej specyfikacji w 2005 roku. Utworzyła również schemat, na którm można opierać się przy tworzeniu BMM (rys. \ref{BMM_schem})\cite{AnaWesHal}.

\begin{figure}[h!tbp]
\begin{centering}
\includegraphics[width=12cm]{img/BMM.png}
